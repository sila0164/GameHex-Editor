...
- # end
  ...
  `end`
  The end command tells the program when an array, function or list ends, when these are added directly in a script.
...
- # Commands
  ...
  - ## @ (Moving the offset)
    ...
    - ## repeat
      ...
      All `@` lines can be repeated by adding `repeat` to the line.  

      `repeat 2` 
      `repeat -1`

      `repeat` needs a value after it. It can be a limited number so that it only repeats a specified amount of times.
      `-1` can be used when it should repeat for the entire file.
...
# Arrays:
  ...
  Arrays are a way to predetermine a bunch of values. Arrays are similar to functions, but can only contain `@ read` commands. Arrays can be removed/added dynamically by using the `removable command`, and will create a node automatically containing all the values of the array.  
